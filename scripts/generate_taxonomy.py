...
def add(type_, name, parent=None, level=1, aliases=None, prefix=None, note=None):
    ...
    # slug == id, deliberately, not a stripped/bare version — the tree has
    # genuine cross-type name reuse ("Operations" as both dom-operations and
    # fam-operations, "Inspection" as vertical/domain/family) documented in
    # TALENT_BANK_PHASE0.md §1's "name collisions across layers" rule.
    # Stripping the type-prefix to get a bare slug would silently collide 14
    # different concepts onto the same string — confirmed by testing it
    # against the full node set before making this change. Matches
    # categorySeed.ts's id===slug convention while correctly accounting for
    # this taxonomy having multiple node types where names legitimately repeat.
    node = {"id": nid, "slug": nid, "type": type_, "name": name, "parentId": parent,
...
IND = [
 ...
 ("Renewables", ["Hydrogen","Green Ammonia","Biofuels","Renewable Diesel","Sustainable Aviation Fuel","Carbon Capture","Carbon Storage","Battery Energy Storage","Carbon Markets","Solar","Wind","Nuclear"]),
...
VERT = [("Terminal Operators",["Independent Storage Operator"]),("EPC Contractors",["Engineering Procurement Construction","EPC"]),
 ...
 ("Oilfield Services",["Well Services","Drilling Contractors","Wireline Services","Completion Services"]),
]
...
DOM = [
 ...
 ("ESG / Sustainability", ["Emissions Reporting","Carbon Accounting","Sustainability Strategy","ESG Reporting","Climate Risk","Decarbonization"]),
 ("Drilling & Rig", ["Drilling Engineering","Well Services","Rig Operations","Directional Drilling","Mud Engineering","Wireline"]),
 ("Geosciences", ["Geology","Geophysics","Petrophysics","Reservoir Engineering","Seismic Interpretation"]),
]
